...
int download_stream(char *p_charbuf, char *dir)
{
	...
	strcat(cmd, " && /usr/bin/mpc update --wait && VV=$(ls *.description -t | head -n1) && VV=$(basename $VV .description) && /usr/bin/mpc add youtube/$VV.m4a && /bin/echo $VV && /usr/bin/find -type f -atime +14 -delete");
